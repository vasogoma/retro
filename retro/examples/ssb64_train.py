...
def step_callback(lcl, _glb):
    ...
    #if lcl['t'] % 1000 == 0:
    #    logging.error(lcl['episode_rewards'])
    #if len(lcl['episode_rewards']) >1:
    #    return True
    
    #if lcl['t'] % 1000 == 0:
    #    logging.error(f"{lcl['t']}: {lcl['episode_rewards']}")
    ...
    if is_done and lcl['episode_rewards'][-2] < 10 and epinum > 0: # To visualize only very good replays
        ...
        #else:
            #logging.error(f"The file {filename_to_delete} does not exist")
...
def train_dqn(env,mode,ai_level=1):
    ...
    act=deepq.learn(
        ...
        total_timesteps=step_totals,
        buffer_size=int(step_totals/50), #buffer of 1/50: 2% 
...
def test_abel(char_list=["pikachu","mario", "dk","link","samus","kirby","yoshi", "fox" ], num_eps=200):
    rewards = []
    matches_data = []
    victory = False
    for i in range (num_eps):
        try:
            ...
            env = make_env("ai", char_list=char_list)
            ...
            while(True):
                ...
                if j < 10:
                    ...
                    actions=[abel.policy(state)] # take an action for player
                j+=1
    ...
    matches_dataframe.to_csv("abel_results.csv")
...
def run_sim(env):
    ...
    while(True):
        ...
        #    logging.error(state)
        #    logging.error("------------------")
        #    logging.error(f"Step: {i}")
        #    logging.error(f"C: {state[0]}, P: ({state[1]},{state[2]}), V: ({state[3]},{state[4]}), MS: {state[5]}, MF: {state[6]}, D: {state[7]}, DMG: {state[8]}")
        #    logging.error(f"C: {state[9]}, P: ({state[10]},{state[11]}), V: ({state[12]},{state[13]}), MS: {state[14]}, MF: {state[15]}, D: {state[16]}, DMG: {state[17]}")
        #    logging.error(f"Action: {action2}")
        # Match ends if terminal is true (someone won)
        if terminal:
        #    logging.error(f"Action: {action2}")
        #    plt.imshow(state_img) # show the image of the state
        #    plt.savefig(f"pics/color{i}.png") # save the image
        #    logging.error(f"reward: {reward}") # print the reward
            break
    #env.stop_record()

def main(command="train_pikachu"):
